"""
첫 30주 피처로 31주~끝 구간의 매출 기울기(slope_after_30w)를 회귀 예측.
- Ridge, XGBRegressor 사용. 8:2 매장 분할, RMSE/MAE/R² 평가.
- 선택: 회귀 예측값을 임계값 0으로 이진화해 성장/하락 F1도 보고 (regression-then-classify).

실행 전 build_30w_features_and_labels.py 를 먼저 실행해야 함 (slope_after_30w 컬럼 필요).
"""
...
LOG_PATH = ROOT / "outputs" / "logs" / "run_regression_30w.log"
...
ID_COL = "public_id"
TARGET_CONTINUOUS = "slope_after_30w"
TARGET_BINARY = "growth"
CLUSTER_COL = "cluster"
...
def main():
    ...
    if TARGET_CONTINUOUS not in df.columns:
        log(f"ERROR: Need '{TARGET_CONTINUOUS}'. Re-run build_30w_features_and_labels.py to generate it.")
        return
    ...
    y_slope = df[TARGET_CONTINUOUS].astype(float).values
    y_growth = df[TARGET_BINARY].astype(int).values if TARGET_BINARY in df.columns else None
    ...
    X_train, X_test, y_slope_train, y_slope_test = train_test_split(
        X, y_slope, test_size=1 - train_ratio, random_state=seed
    )
    test_idx = X_test.index
    y_growth_test = df.loc[test_idx, TARGET_BINARY].values if (y_growth is not None and TARGET_BINARY in df.columns) else None
    ...
    for name, model in [
        ...
        if name == "XGBRegressor":
            try:
                ...
                continue
        model.fit(X_train_s, y_slope_train)
        pred = model.predict(X_test_s)
        rmse = np.sqrt(mean_squared_error(y_slope_test, pred))
        mae = mean_absolute_error(y_slope_test, pred)
        r2 = r2_score(y_slope_test, pred)
        results.append({
            "spec": "M0", "model": name,
            "rmse": rmse, "mae": mae, "r2": r2,
        })
        log(f"M0 {name} — RMSE: {rmse:.4f}, MAE: {mae:.4f}, R²: {r2:.4f}")
        if y_growth_test is not None:
            pred_bin = (pred >= 0).astype(int)
            f1 = f1_score(y_growth_test, pred_bin, zero_division=0)
            results[-1]["f1_from_regression"] = f1
            log(f"  regression→binary (threshold=0) F1: {f1:.4f}")

    # M1: 피처 + 클러스터 더미 (30주로 예측한 클러스터)
        ...
        for name, model in [
            ...
            if name == "XGBRegressor":
                try:
                    ...
                    continue
            model.fit(X_train_m1, y_slope_train)
            ...
            log(f"M1 {name} — RMSE: {rmse:.4f}, MAE: {mae:.4f}, R²: {r2:.4f}")
            if y_growth_test is not None:
                pred_bin = (pred >= 0).astype(int)
                f1 = f1_score(y_growth_test, pred_bin, zero_division=0)
                results[-1]["f1_from_regression"] = f1
                log(f"  regression→binary F1: {f1:.4f}")

    res_df = pd.DataFrame(results)
    out_csv = TABLES_DIR / "regression_30w_results.csv"
    res_df.to_csv(out_csv, index=False)
    log(f"Saved {out_csv}")

    # Figure: R² by spec and model
    if len(res_df) > 0 and "r2" in res_df.columns:
        ...
        ax.set_ylabel("R²")
        ax.set_title("Regression (30w → slope_after_30w): R² by spec & model")
        ax.legend()
        ax.axhline(y=0, color="gray", linestyle="--")
        fig.tight_layout()
        fig.savefig(FIGURES_DIR / "regression_30w_r2.png", bbox_inches="tight")
        plt.close(fig)
        log(f"Saved {FIGURES_DIR / 'regression_30w_r2.png'}")
