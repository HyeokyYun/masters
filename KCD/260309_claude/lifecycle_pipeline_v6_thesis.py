...
# ═══════════════════════════════════════════════════════════
# 5b. 종합 평가 (AUC-ROC, 혼동행렬, 분류 리포트)
# ═══════════════════════════════════════════════════════════
def evaluate_classifier(pipe, X, y, label_names, cv=5):
    """
    다중 지표 평가: F1, Accuracy, ROC-AUC(ovr), 혼동행렬, classification_report
    """
    ...
    cv_res = cross_validate(
        pipe, X, y, cv=cv_split, scoring=scoring,
        return_train_score=False,
    )
    y_pred = cross_val_predict(pipe, X, y, cv=cv_split)
    cm = confusion_matrix(y, y_pred, labels=label_names)
    ...
    return {
        ...
        "roc_auc_std": cv_res["test_roc_auc_ovr"].std(),
        "confusion_matrix": cm,
        "classification_report": classification_report(y, y_pred, labels=label_names, zero_division=0),
    }
...
def early_prediction(ts, feat, W=30):
    ...
    X = edf[fc].astype(float).values
    y = edf["label"].astype(str).values
...
# ═══════════════════════════════════════════════════════════
# 8. SHAP Feature Importance Analysis (Thesis Addition)
# ═══════════════════════════════════════════════════════════
def analyze_shap_importance(ts, feat, W=30, out="outputs/shap_summary_v6.png"):
    print("\n[SHAP Analysis] Calculating SHAP values for feature importance...")
    ...
    # Simple imputation since Random Forest doesn't handle NaNs well natively
    imp = SimpleImputer(strategy="median")
    ...
    # In newer SHAP versions, multi-class shap_values is a 3D array (N, n_features, n_classes)
    # summary_plot expects a list of 2D arrays for multi-class datasets.
    if isinstance(shap_values, np.ndarray) and shap_values.ndim == 3:
        shap_values = [shap_values[:, :, i] for i in range(shap_values.shape[2])]

    # To plot a summary across all classes:
    plt.figure(figsize=(10, 6))
    shap.summary_plot(shap_values, X_imp, feature_names=fc, class_names=clf.classes_, show=False)
    plt.title(f"SHAP Feature Importance (Window: {W} Weeks)", fontweight="bold")
    plt.tight_layout()
    plt.savefig(out, dpi=150, bbox_inches="tight", facecolor="white")
    print(f"  → SHAP plot saved to {out}")
    plt.close()


# ═══════════════════════════════════════════════════════════
# 9. Time Window Sensitivity Analysis (Thesis Addition)
# ═══════════════════════════════════════════════════════════
def run_window_sensitivity(ts, feat, windows=[10, 20, 30, 40, 50], out="outputs/window_sensitivity_v6.png"):
    print("\n[Sensitivity Analysis] Running predictions across multiple time windows...")
    
    os.makedirs(os.path.dirname(out), exist_ok=True) if os.path.dirname(out) else None

    results = []
    for W in windows:
        res = early_prediction(ts, feat, W=W)
        if res:
            results.append(res)
    ...
    plt.errorbar(df_res["W"], df_res["f1"], yerr=df_res["std"], marker='o', 
                 capsize=5, capthick=2, elinewidth=2, color="#1565C0", markersize=8)
    plt.plot(df_res["W"], df_res["f1"], color="#1565C0", alpha=0.5, linestyle="--")
    
    for i, row in df_res.iterrows():
        plt.annotate(f"n={row['n']:,}", (row["W"], row["f1"] - row["std"] - 0.02),
                     ha='center', fontsize=9, color="gray")

    plt.title("조기 예측 성능과 관측 주차(Window) 간의 민감도 분석", fontsize=12, fontweight="bold")
    plt.xlabel("조기 관측 주차 (Weeks since open)")
...
if __name__ == "__main__":
    ...
    print("\n[Step8] SHAP 피처 중요도 분석 (논문용)...")
    analyze_shap_importance(ts, feat, W=30, out="outputs/shap_summary_v6.png")
    
    print("\n[Step9] 관측 기간 민감도 분석 (논문용)...")
    sens_df = run_window_sensitivity(ts, feat, windows=[10, 20, 30, 40, 50], out="outputs/window_sensitivity_v6.png")
    sens_df.to_csv("outputs/window_sensitivity_results_v6.csv", index=False)

    print("\n[Step10] 시각화...")
    visualize(feat, ablation_df, ts, early_res, km_model,
              out="outputs/lifecycle_results_v6.png")

    # ── 최종 요약
    print("\n" + "═"*62)
    print("✅  분석 완료  (v6.0 — 데이터 드리븐 + SHAP & 민감도)")
    print("═"*62)
    vc = feat["label"].value_counts().reindex(LABELS, fill_value=0)
    print(f"\n  {'레이블':<6} {'업장수':>7} {'비율':>7}  설명")
    print("  " + "-"*55)
    ...
    for f in ["lifecycle_features_v5.csv","ablation_results_v5.csv",
              "outputs/lifecycle_results_v6.png", "outputs/shap_summary_v6.png", "outputs/window_sensitivity_v6.png"]:
        if os.path.exists(f):
