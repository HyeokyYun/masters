...
Run from 26-1: ./.venv/bin/python 260320_codex/run_01_trend_residual_cv.py
"""
import numpy as np
import pandas as pd
from datetime import datetime

from config import WEEKLY_PARQUET, OUT_DIR, LOG_DIR, MIN_WEEKS_FOR_TREND, resolve_multinom_path
...
def main():
    ...
    weekly_cols = ["public_id", "sales_card"]
    if pq is not None:
        schema_cols = set(pq.read_schema(WEEKLY_PARQUET).names)
        if "day_after1" in schema_cols:
            weekly_cols.append("day_after1")
        elif "date_id" in schema_cols:
            weekly_cols.append("date_id")
    else:
        weekly_cols.append("date_id")
    weekly = pd.read_parquet(WEEKLY_PARQUET, columns=weekly_cols)
