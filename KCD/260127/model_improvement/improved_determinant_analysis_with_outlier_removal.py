"""
Improved Determinant Analysis WITH Outlier Removal
...
print("=" * 80)
print("Improved Determinant Analysis (WITH Outlier Removal)")
print("=" * 80)
...
# 2-2. 강화된 이상치 처리
print("  - Enhanced outlier detection and removal...")
...
for col in numeric_cols:
    if col in df_encoded.columns:
        ...
        removed = initial_count - len(df_encoded)
        if removed > 0:
            outlier_removed_count += removed
            print(f"    {col}: {removed} outliers removed (IQR)")

print(f"  - 총 {outlier_removed_count}개 이상치 제거됨")

# 윈저화 (상하위 0.5%)
print("  - Winsorization (0.5% and 99.5%)...")
for col in numeric_cols:
    if col in df_encoded.columns:
...
plt.tight_layout()
plt.savefig('results/confusion_matrices_with_outlier_removal.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.tight_layout()
plt.savefig('results/feature_importance_with_outlier_removal.png', dpi=300, bbox_inches='tight')
plt.close()

print("  Saved: confusion_matrices_with_outlier_removal.png, feature_importance_with_outlier_removal.png")
...
performance_df.to_csv('results/model_performance_with_outlier_removal.csv', index=False, encoding='utf-8-sig')
print("  Saved: model_performance_with_outlier_removal.csv")

# 변수 중요도 저장
rf_importance.to_csv('results/rf_feature_importance_with_outlier_removal.csv', index=False, encoding='utf-8-sig')
xgb_importance.to_csv('results/xgb_feature_importance_with_outlier_removal.csv', index=False, encoding='utf-8-sig')
print("  Saved: rf_feature_importance_with_outlier_removal.csv, xgb_feature_importance_with_outlier_removal.csv")
...
for name, y_pred in [('Logistic_Regression', y_pred_lr), 
    ...
    report_df.to_csv(f'results/classification_report_{name}_with_outlier_removal.csv', 
                     encoding='utf-8-sig')
    print(f"  Saved: classification_report_{name}_with_outlier_removal.csv")

# ============================================================================
# 8. 요약
# ============================================================================
print("\n" + "=" * 80)
print("Improved Analysis Complete! (WITH Outlier Removal)_Server")
print("=" * 80)
print(f"\nModel Performance:")
