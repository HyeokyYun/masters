"""
LSTM Experiment for Sales Prediction
- 단기 예측 (1-4주)
- 장기 예측 (6개월, 2년)
- 성능 평가 및 비교
"""
...
try:
    ...
    tf.get_logger().setLevel('ERROR')  # ERROR 레벨만 출력
except ImportError:
    print("ERROR: TensorFlow가 설치되지 않았습니다.")
    print("다음 명령어로 설치해주세요: pip install tensorflow")
    sys.exit(1)

# GPU 선택 (원하는 GPU 번호 지정)
USE_GPU_ID = 1  # 0, 1, 2, 3 중 선택 가능
...
print("=" * 80)
print("LSTM Experiment for Sales Prediction")
print("=" * 80)
...
for horizon in prediction_horizons:
    ...
    # GPU 사용 확인
    if gpus and USE_GPU_ID is not None and USE_GPU_ID < len(gpus):
        try:
