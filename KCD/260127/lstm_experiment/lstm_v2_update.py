...
# 1-1. 매출액 로그 변환 (매우 중요: MAPE를 낮추는 핵심)
weekly_df['sales_card'] = np.log1p(weekly_df['sales_card'])

# 1-2. 클러스터 번호 원-핫 인코딩 (0~8번을 9개 컬럼으로 확장)
cluster_dummies = pd.get_dummies(weekly_df['cluster'], prefix='cluster')
weekly_df = pd.concat([weekly_df, cluster_dummies], axis=1)

# 1-3. 노이즈 제거를 위한 4주 이동 평균 추가
weekly_df['sales_rolling_4'] = weekly_df.groupby('public_id')['sales_card'].transform(lambda x: x.rolling(4).mean())
weekly_df = weekly_df.fillna(0) # 결측치 처리

# 1-4. 사용할 최종 특성 정의
cluster_cols = [col for col in weekly_df.columns if col.startswith('cluster_')]
features = ['sales_card', 'sales_rolling_4'] + cluster_cols
num_features = len(features)

print(f"✅ 특성 생성 완료: 총 {num_features}개 특성 사용")
...
def create_sequences(data, sequence_length, prediction_horizon, feature_cols):
    X, y = [], []
    ...
    for pid in public_ids:
        ...
        # 특성 데이터(매출+클러스터 등)와 타겟 데이터(미래 매출) 분리
        f_values = store_data[feature_cols].values
        s_values = store_data['sales_card'].values # 로그화된 매출
        
        if len(f_values) < sequence_length + prediction_horizon:
            continue
        
        for i in range(len(f_values) - sequence_length - prediction_horizon + 1):
            X.append(f_values[i : i + sequence_length])
            y.append(s_values[i + sequence_length : i + sequence_length + prediction_horizon])
...
for horizon in prediction_horizons:
    ...
    # 시퀀스 생성
    X, y = create_sequences(weekly_df, sequence_length, horizon, features)
    ...
    with tf.device(f'/GPU:{USE_GPU_ID}'):
        model = Sequential([
            # input_shape의 마지막 차원을 1에서 num_features로 변경
            tf.keras.layers.Bidirectional(LSTM(128, return_sequences=True), input_shape=(sequence_length, num_features)),
            Dropout(0.3),
            LSTM(64, return_sequences=False),
            Dropout(0.2),
            Dense(32, activation='relu'),
            Dense(horizon, activation='linear') # 로그 값이므로 linear 사용
        ])
    ...
    # 1. 스케일러 역변환
    y_pred_log = scaler_y.inverse_transform(y_pred_reshaped)
    y_true_log = scaler_y.inverse_transform(y_test_reshaped)

    # [수정] 무한대(inf) 방지 로직 추가
    # 1) float64로 변환하여 연산 정밀도 확보
    y_pred_log = y_pred_log.astype(np.float64)
    y_true_log = y_true_log.astype(np.float64)
    
    # 2) 비정상적으로 큰 값(Log scale 30 이상)은 자름 (e^30은 약 10조)
    y_pred_log = np.clip(y_pred_log, None, 30)
    ...
    if horizon == 1:  # 1주 예측만 시각화
        ...
        for i in range(min(5, len(y_test))):
            axes[1, 0].plot(range(sequence_length), X_test[i, :, 0], label=f'Input {i+1}', alpha=0.7)
        ...
        plt.tight_layout()
        plt.savefig(f'results/short_term_prediction_{horizon}week_v2_update.png', dpi=300, bbox_inches='tight')
        plt.close()
        print(f"    Saved: short_term_prediction_{horizon}week_v2_update.png")
...
for horizon in long_term_horizons:
    ...
    # 시퀀스 생성 (features 인자 추가 필수)
    X, y = create_sequences(weekly_df, sequence_length_long, horizon, features)
    ...
    plt.tight_layout()
    plt.savefig(f'results/long_term_prediction_{horizon}weeks_v2_update.png', dpi=300, bbox_inches='tight')
    plt.close()
    print(f"    Saved: long_term_prediction_{horizon}weeks_v2_update.png")
...
if short_term_results:
    ...
    short_term_df.to_csv('results/short_term_prediction_results_v2_update.csv', index=False, encoding='utf-8-sig')
    print("  Saved: short_term_prediction_results_v2_update.csv")

# 장기 예측 결과
if long_term_results:
    long_term_df = pd.DataFrame(long_term_results)
    long_term_df.to_csv('results/long_term_prediction_results_v2_update.csv', index=False, encoding='utf-8-sig')
    print("  Saved: long_term_prediction_results_v2_update.csv")
