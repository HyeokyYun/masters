...
업종 × 동 수준(§\ref{sec:industry_dong})에서 전구간 궤적 형상 구성은 단순 업종 평균·동 평균으로 가려지는 체계적 차이를 보인다. 가장 성장형인 셀은 주거 구의 퀵서비스(예: 강동구 치킨 성장형 57%), 가장 쇠퇴형인 셀은 도심 구의 점내·주류 업종(예: 중구 한식 일반 쇠퇴형 71%)이다. 동일 업종 내에서도 성장형 비중은 동별로 0.23~0.41로 변동한다. 이 *공간 × 산업* 구조는 단순 평균으로는 드러나지 않고 전구간 형상으로 정의돼 패널 선택에 robust하며, 본 논문이 도출한 해석적·실무적 가치의 한 축이다.
