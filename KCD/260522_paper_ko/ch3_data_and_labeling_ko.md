...
본 장은 본 학위논문에서 사용한 데이터의 구조, 분석을 서울 외식업으로 한정한 근거, G/S/D 라벨의 정의와 임계값 민감도, 평가 지표 선택의 근거, 분석 단위(점포 / 업종 × 구), 전처리 절차, 학습·평가 분할 프로토콜을 기술한다.
...
- **점포 메타데이터** (`original_data/meta.csv`). 업종 코드, 자치구, 사업 개시일(영업기간 산출용), 사업 형태 등의 tabular 정보.

매출 시계열의 주기는 **주간(중빈도)** 이다. 일 단위의 noise는 smoothing되면서 월 단위보다는 dense하여 변화점 탐지에 유리하며, 본 논문의 평가 윈도우 길이(분기 ≈ 약 13주)와도 자연스럽게 정렬된다.

데이터 접근은 KCD와의 공동연구 협약 하에서 제공되었다. `store_id`, 자치구 코드, 업종 코드는 모두 **비식별 처리된 형태로만** 사용되며, 상호명·사업자등록번호·위치 좌표 같은 직접 식별 정보는 본 분석에 사용되지 않는다.
...
3. **충분한 cell 두께.** 업종 × 구 조합 수준 분석(Chapter 4)에서, 통계적 유의성 임계값 이상의 점포 수를 가지는 cell이 충분히 많다. 서울의 25개 자치구와 본 논문이 식별하는 약 30개 외식 세부 업종의 조합은 $n \geq 100$ 점포를 갖는 다수의 cell을 산출한다.
...
| District | Ch 4.2 | 구 단위 G/S/D 분포 |
| Industry × District | Ch 4.2 | 공간 × 산업 조합 패턴 |

특히 업종 × 구 단위는 본 논문이 강조하는 focal analysis로, "어떤 조합이 Growth 비율이 높고 Decline 비율이 높은가" 를 도출하는 데 사용된다 (§\ref{sec:industry_dong}).
