"""
전체 기간 데이터로 전체 총 매출(total_sales)을 설명하는 OLS 회귀.
- y = total_sales = avg_sales_card * total_weeks (매장별 전체 기간 총 매출).
- X = 그 외 모든 피처 (avg_sales_card, total_weeks 제외하여 회귀의 의미 유지).

Run from 260204: python scripts/run_03_regression_total_sales.py
실행 전 run_01_prepare_features 로 data_features_clean.parquet 생성 필요.
"""
...
LOG_PATH = ROOT / "outputs" / "logs" / "run_03_regression_total_sales.log"
OUT_CSV = ROOT / "outputs" / "tables" / "ols_total_sales.csv"
...
def main():
    ...
    # 전체 총 매출 = 평균 매출 * 주수 (매장별)
    if "avg_sales_card" not in df.columns or "total_weeks" not in df.columns:
        log("ERROR: Need 'avg_sales_card' and 'total_weeks' to compute total_sales.")
        return
    df["total_sales"] = df["avg_sales_card"] * df["total_weeks"]

    y_col = "total_sales"
    # 종속변수와 직접 정의에 쓰인 변수 제외한 피처만 사용 (어떤 요인이 총 매출과 연관되는지 해석)
    exclude = {id_col, cluster_col, y_col, "avg_sales_card", "total_weeks"}
    ...
    log(f"total_sales: mean = {y.mean():.0f}, std = {y.std():.0f}")
