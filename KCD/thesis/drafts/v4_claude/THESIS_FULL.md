...
- **지도교수**: KAIST 김지희
- **학위**: 석사 (논문)
- **데이터**: KCD 서울시 외식업 주간 카드거래 패널 (2021-01-01 ~ 2023-08-28,
  142주, 약 5만 9천 점포)
...
| 장 | 파일 | 핵심 메시지 |
| --- | --- | --- |
| 초록 | [`ch0_abstract.md`](ch0_abstract.md) | 한/영 초록. cluster+CP는 조건부 contribution이며 시즌 정렬 robustness 자체가 두 contribution. |
| 1장 서론 | [`ch1_introduction.md`](ch1_introduction.md) | 미팅에서 제기된 두 쟁점, RQ1–RQ4, 두 contribution 명시. |
| 2장 선행연구 | [`ch2_literature_review.md`](ch2_literature_review.md) | 라이프사이클, 거래 데이터 진단, 시계열 분류, 시즌 통제 robustness 네 갈래. |
| 3장 데이터 | [`ch3_data.md`](ch3_data.md) | KCD weekly panel 변수, 전처리, 80 specification 카탈로그. |
| 4장 방법론 | [`ch4_methodology.md`](ch4_methodology.md) | 시즌 정렬 rolling-window, G/S/D 라벨, 가변 윈도우 피처, A/B/C/D 모델, paired t-test. |
| 5장 결과 | [`ch5_results.md`](ch5_results.md) | 80 specification baseline + 7 panel × A/B/C/D, Δ(D−A) = +0.0022, 1/7 유의. |
| 6장 논의 | [`ch6_discussion.md`](ch6_discussion.md) | 시즌 confound 함의, hybrid representation 외적 타당성, 한계, future work 분리. |
| 7장 결론 | [`ch7_conclusion.md`](ch7_conclusion.md) | 요약 + 후속 4가지 (검정력 보강, 신규 고객 선행성 재검증, 메서드 정교화, 외적 타당도). |
...
- 각 장은 독립적으로 읽을 수 있도록 작성됐다. 빠른 검토는 ch5 → ch6 →
  ch0(abstract) 순으로 보면 핵심 발견과 framing이 빠르게 보인다.
- 결과 검증 시 `260430_claude/outputs/tables/*.csv`의 원본 수치를 직접 대조
  한다(특히 §5.5의 표).
