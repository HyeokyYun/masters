...
분석 결과, 초기 거래 패턴은 이후 점포 상태 예측에 의미 있는 정보를 제공한다. 기존 full-window 예측에서는 trajectory 및 change-point 정보를 결합한 hybrid representation이 기본 feature 대비 성능을 개선하였다. 또한 계절성 보정 rolling-window 검증에서도 예측 신호는 완전히 사라지지 않았으며, 최고 성능 specification은 Macro-F1 0.509, AUC 0.722, Decline recall 0.663을 보였다. 더 보수적인 strict out-of-time rolling 검증에서는 평균 Macro-F1 0.407, AUC 0.650으로 성능이 약화되었지만, majority baseline을 상회하였다. 이는 카드거래 기반 조기 신호가 단순히 특정 계절 비교의 산물은 아니지만, 연도 간 일반화에는 한계가 있음을 시사한다.
