...
│   ├── step04_label_assignment.py   ← 6-class 레이블 할당 (전체추세 기반)
...
### Step 04 — 레이블 할당 (12-class, data-driven)
```
slope_early × slope_late × slope_all(3단계) → 12개 클래스

표기법:
  1st letter = 전반기 추세 (U=상승, D=하락)  ← slope_early_mm 부호
  2nd letter = 후반기 추세 (U=상승, D=하락)  ← slope_late_mm 부호
  suffix     = 전체 추세 방향               ← slope_all_mm vs ±threshold
               _X = 전체 추세 상승 (slope_all > +threshold)
               _Y = 전체 추세 유지 (|slope_all| ≤ threshold)
               _Z = 전체 추세 하락 (slope_all < -threshold)

  threshold = std(slope_all_mm) × 0.5  (SLOPE_ALL_THRESHOLD_FACTOR)
...
**260316 버전과의 차이**:
1. 6 → 12 클래스 (모든 패턴에 X/Y/Z 적용)
2. DD/UD 세분화 기준: MDD(최대 낙폭) → slope_all_mm(전체 추세 기울기)
3. X(상승)/Y(유지)/Z(하락) 3단계 분류 (std×0.5 기반 유지 대역)
